...
<form action="" method="post" class="ajaxform">
 	...
 	<legend> Detail Karyawan</legend>
                	...
                	<input type="hidden" name="score_taker_id" value="<?php echo $data['score_taker_id'] ?>" />	
					</div>
					<div>
                    <label>Jabatan</label>
                    ...
                    <label>Periode Penilaian(Awal)</label>
			            <input type="text"  value="<?php echo mysql_to_human($data['period_start']) ?>" class="ac_hidden" />
                     	
                    </div>
                    <div>
                    <label>Periode Penilaian(Akhir)</label>
                             <input type="text"  value="<?php echo mysql_to_human($data['period_end']) ?>" class="ac_hidden" />
                    </div>
    ...
    <legend>Produktifitas</legend>
                    ...
                    <label>Membuat target yang realistis</label>
                    	<?php echo xform_radio('prod_create_target',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Memenuhi batas waktu</label>
                    		<?php echo xform_radio('prod_achive',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mencari efisiensi</label>
                    	<?php echo xform_radio('prod_good_job',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mampu menangani banyak project bersamaan</label>
                    <?php echo xform_radio('prod_concurrent_project',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Produktifitas pekerjaan rutin</label>
                    <?php echo xform_radio('prod_routine_activity',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Waktu respon masalah</label>
                    <?php echo xform_radio('prod_respon_time',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Waktu penyelesaian masalah</label>
                    <?php echo xform_radio('prod_resolution',$criteria_options,$data,'') ?>
                    </div>
                    
                    <div>
                         <label>Endurance(ketahanan)</label>
                         <?php echo xform_radio('prod_endurance',$criteria_options,$data,'') ?>
                    </div>
               </fieldset>
               <fieldset id="com">
               	<legend>Komunikasi</legend>  
                    ...
                    <?php echo xform_radio('com_inform_process',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mendengarkan pihak lain </label>
                    	<?php echo xform_radio('com_listening',$criteria_options,$data,'') ?>
                    </div>
                     <div>
                    <label>Etika Berkomunikasi</label>
                    <?php echo xform_radio('com_ethic',$criteria_options,$data,'') ?>
                    </div>
                    
                    <div>
                    <label>Berkomunikasi terstruktur</label>
                    <?php echo xform_radio('com_structure',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Kemampuan berbicara didepan umum</label>
                    <?php echo xform_radio('com_front',$criteria_options,$data,'') ?>
                    </div>
                    
                    <div>
                    <label>Kemampuan Verbal</label>
                    <?php echo xform_radio('com_verb',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Kemampuan tulisan</label>
                    <?php echo xform_radio('com_write',$criteria_options,$data,'') ?>
                    </div>
                    
                     <div>
                    <label>Kemampuan Verbal(Bahasa asing)</label>
                    <?php echo xform_radio('com_verb_a',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Kemampuan tulisan(Bahasa asing)</label>
                    <?php echo xform_radio('com_write_a',$criteria_options,$data,'') ?>
                    </div>
                </fieldset>
               ...
               <fieldset id="lead">
                    ...
                    <?php echo xform_radio('lead_example',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mencari solusi yang realistis</label>
                    <?php echo xform_radio('lead_solution',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Bertindak tegas, menghadapi masalah scr langsung</label>
                    <?php echo xform_radio('lead_firm',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mengupayakan yang terbaik untuk anggota tim</label>
                    <?php echo xform_radio('lead_best',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Menyelesaikan Konflik</label>
                    <?php echo xform_radio('lead_solve_conflict',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Membuat harapan yang jelas</label>
                    <?php echo xform_radio('lead_clear_hope',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mendelegasikan dengan jelas</label>
                    <?php echo xform_radio('lead_delegate',$criteria_options,$data,'') ?>
                    </div>
                     </fieldset>
               <fieldset id="prive">
                    ...
                    <?php echo xform_radio('prive_cool',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Menetapkan standar yang tinggi u/ diri sendiri</label>
                    <?php echo xform_radio('prive_high_std',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Menetapkan tujuan yang menantang</label>
                    <?php echo xform_radio('prive_challenge',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Kreatif dan inisiatif</label>
                    <?php echo xform_radio('prive_creative',$criteria_options,$data,'') ?>
                    </div>
               </fieldset>
               ...
               <fieldset id="rel">
                    ...
                    <?php echo xform_radio('rel_client',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mengesampingkan prasangka</label>
                    <?php echo xform_radio('rel_assumtion',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Memiliki semangat kerjasama tim(TeamWork)</label>
                    <?php echo xform_radio('rel_teamwork',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Memberi nasihat yang baik dan praktis</label>
                    <?php echo xform_radio('rel_suggest',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                      <label>Memiliki loyalitas</label>
                    <?php echo xform_radio('rel_loyalty',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                      <label>Memiliki tanggung jawab</label>
                    <?php echo xform_radio('rel_responsibility',$criteria_options,$data,'') ?>
                    </div>
               ...
               <fieldset id="mng">
                    ...
                    <?php echo xform_radio('mng_job_priority',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Menangani permasalahan dengan cepat dan baik</label>
                    <?php echo xform_radio('mng_problem',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mengembangkan strategi baru</label>
                    <?php echo xform_radio('mng_new_strategy',$criteria_options,$data,'') ?>
                    </div>
                    <div>
                    <label>Mengatur Tugas</label>
                    <?php echo xform_radio('mng_role_set',$criteria_options,$data,'') ?>
                    </div>
                    </fieldset>
                   <fieldset> 
                    ...
                    <label>Rata-rata</label>
                          <input type="text" value="<?php echo $data['average']?>" name="average" id="average" class="number"  />
                    </div>
                    <div>
                    <label>Catatan</label>
                          <textarea rows="" cols=""  name="notes">
                          <?php echo $data['notes']?>
                          </textarea>
                    </div>
                    <div>
                    <label>Tanggal Penilaian</label>
                    	<input type="text"  value="<?php echo mysql_to_human($data['judge_date']) ?>" class="ac_hidden" />
                    </div>
				</fieldset>

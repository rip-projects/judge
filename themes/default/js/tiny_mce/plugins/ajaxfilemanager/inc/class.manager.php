...
class manager {
    ...
    function __construct($path = null, $calculateSubdir=true) {
        ...
        if ($this->calculateSubdir) {// keep track of this folder path in session 
        ...
        if ($dirHandler) {
            while (false !== ($file = readdir($dirHandler))) {
                if ($file != '.' && $file != '..') {
                    ...
                    if ($this->sessionAction->getFolder() == $this->currentFolderPath) {//check if any flag associated with this folder or file
